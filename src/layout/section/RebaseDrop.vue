<template>
  <div class="card bg-dark" style="min-height: 400px">
    <h5 class="card-header text-white">Git | Interactive Rebase</h5>
    <div class="card-body bash-console text-white">
      <p class="card-text">Makes it possible to edit commits before rebasing</p>
      <p class="card-text">Creates new commits for the edited commits / commits which history has been changed</p>
      <p class="card-text">Options: reword | edit | squash | fixup | exec | drop</p>
      <div class="d-flex git-command-body">
          ...
          <div class="git-commit-screen">
            ...
            <git-transition @startEnter="startEnter" @startLeave="startLeave">
              <div v-show="!complete">
                ...
                <git-commit-box><span ref="commit2">{{commitId2}}</span></git-commit-box>
              </div>
            </git-transition>
          </div>
        </div>
        <git-bash style="min-height: 180px">
          <bash-command v-if="step[0]" :command="command[0]" @onComplete="nextStep">
            <p class="mb-0 ml-3"><span ref="pick">{{mode}}</span><span class='ml-3'>ec5bef2 My first message!</span></p>
            <p class="mb-0 ml-3"><span>pick</span><span class='ml-3'>e45cb9e My second message!</span></p>
  ...
  methods: {
    ...
    startLeave () {
      ...
      t3.to(this.$refs.commit2, 0.5, {
        opacity: 0,
        onComplete: () => { this.commitId2 = 'c4ec9' }
